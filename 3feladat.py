...
while True:
    be = input("Adj meg egy betűt (ENTER = kilépés): ")
    ...
    betu_upper = betu.upper()
    tarolt_upper = [b.upper() for b in betuk]

    if betu_upper in tarolt_upper:
        print("Ezt a betűt már rögzítettem.")
    else:
        betuk.append(betu)

    
    rendezett = sorted(betuk, key=lambda x: (x.islower(), x.upper()))
print("Rögzített betűk abc sorrendben:", rendezett)
